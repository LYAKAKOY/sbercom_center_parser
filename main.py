...
def parse_table(name_file: str, html_content: str, mode_show: int) -> None:
    soup = BeautifulSoup(html_content, 'html.parser')
    if mode_show == 1:
        table = soup.find('table', class_='table-bordered table-striped table-sm')
    else:
        table = soup.find('table', class_='table-fixed-columns table-bordered table-striped table-sm')
    if table is None:
    ...
    for number, tr in enumerate(trs, start=1):
        for alpha_number, td in enumerate(tr.find_all('td'), start=0):
            sheet[f'{string.ascii_uppercase[alpha_number]}{number}'] = td.text
            print(td.text, end=' ')
        print()
    workbook.save(f'{name_file}.xlsx')
...
try:
    ...
    regions = driver.find_element(By.TAG_NAME, 'select').find_elements(By.TAG_NAME, 'option')
    ...
    for block in left_blocks:
        ...
        if link.text == 'Календарь выборов':
            ...
            label_selects = ['Уровень выборов', 'Вид выборов / Референдумов', 'Тип выборов', 'Система выборов']
            i = 0
            ...
            for item in list_group_item:
                ...
                if child_div:
                    ...
                    if len(nested_divs) == 2:
                        print(number, ': ', nested_divs[0].text or ' ' * 50, ' | ', nested_divs[1].text)
                        print('-' * 1000)
            ...
            wait = WebDriverWait(driver, TIME_WAIT)
            wait.until(EC.presence_of_element_located((By.CSS_SELECTOR, '.nav-link.active.show')))
            ...
            while True:
                try:
                    options = selected_item.find_element(By.XPATH, "..").find_element(By.TAG_NAME, 'ul').find_elements(
                        By.TAG_NAME, 'li')
                ...
                for number, option in enumerate(options, start=2):
                    choices.setdefault(number, option.find_elements(By.TAG_NAME, 'a')[-1])
                    print(number, ':', option.text)
                ...
                if choice == 0:
                    ...
                    selected_li = soup.find(class_='selected-li').find_previous('li').find_previous('ul').find_previous(
                        'li').find_all('a', href=True)
                    selected_item = driver.find_element(By.CLASS_NAME, 'selected-li')
                    if selected_li:
                        href = selected_li[0]['href']
                        previous_selected_item = selected_item.find_element(By.XPATH, '../../..').find_element(By.XPATH,
                                                                                                               f'//a[@href="{href}"]')
